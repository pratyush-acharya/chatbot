...
export default function ConfigSideNav() {
  ...
  const ingestData = async () => {
    try {
      setIsLoading(true);
      const res = await fetch("https://naasa-chat-bak.waterflowtechnology.net/ingest");
      ...
      response.text().then((text) => {
        toast.error("Error Incgesting data." + text);
      });
    }
  };
  ...
  const handleDownloadModel = async () => {
    try {
      ...
      const res = await fetch("https://naasa-chat-bak.waterflowtechnology.net/download_model");
      const jsonData = await res.json();
      if (!res.ok) {
        response.text().then((text) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await fetch("https://naasa-chat-bak.waterflowtechnology.net/upload_doc", {
        method: "POST",
        body: formData,
      });
